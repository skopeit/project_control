...
The system is designed to manage the “compliance functions” of a hardware and/or software implementation on a medium to large scale. It is meant to be collaborative in nature and automate the workflow, approvals, monitoring, and prioritisation, auditing and reporting on the following modules.
...
skopeit relies extensively on environment settings which **will not work with Apache/mod_wsgi setups**. It has been deployed successfully with both Gunicorn/Nginx and even uWSGI/Nginx.

For configuration purposes, the following table maps the 'skopeit' environment variables to their Django setting:
...
DJANGO_DEFAULT_FROM_EMAIL               DEFAULT_FROM_EMAIL          n/a                                            "skopeit <noreply@example.com>"
DJANGO_SERVER_EMAIL                     SERVER_EMAIL                n/a                                            "skopeit <noreply@example.com>"
DJANGO_EMAIL_SUBJECT_PREFIX             EMAIL_SUBJECT_PREFIX        n/a                                            "[skopeit] "
...
.. code-block:: bash
    ...
    cd skopeit
    celery -A skopeit.taskapp worker -l info
...
To run the tests, enter the skopeit/tests directory and run the following commands::
...
Postgres is saving its database files to `/data/skopeit/postgres` by default. Change that if you wan't
...
Place it in `/etc/supervisor/conf.d/skopeit.conf` and run::

    supervisorctl reread
    supervisorctl start skopeit
